#!/usr/bin/env python3
"""
Text Breakdown Tool - Final Version

Based on careful analysis of the provided example, this tool implements
the exact hierarchical breakdown pattern shown for the GET method.
"""
...
class TextBreakdown:
    def __init__(self):
        # Stop words that shouldn't appear alone but may be part of phrases
        self.stop_words = {
            'a', 'an', 'and', 'are', 'as', 'at', 'be', 'by', 'for', 'from', 
            'has', 'he', 'in', 'is', 'it', 'its', 'of', 'on', 'or', 'that', 
            'the', 'to', 'was', 'were', 'will', 'with', 'do', 'does', 'can', 'may'
        }
        
    def identify_sections(self, text: str) -> List[str]:
        """Split text into logical sections based on clear boundaries."""
    ...
    def breakdown_section(self, section_text: str) -> List[str]:
        """
        Break down a section following the exact pattern from the GET example.
        """
        breakdown = []
        
        # 1. Start with the complete section
        breakdown.append(section_text.strip())
        
        # 2. Extract sentences and major components
        sentences = self.extract_sentences(section_text)
        
        for sentence in sentences:
            if sentence.strip() == section_text.strip():
                continue  # Skip if same as full section
                
            breakdown.append(sentence.strip())
            
            # 3. Break down sentence into meaningful phrases
            phrases = self.extract_phrases(sentence)
            for phrase in phrases:
                if phrase not in breakdown:
                    breakdown.append(phrase)
            
            # 4. Extract key terms and words
    ...
    def extract_phrases(self, sentence: str) -> List[str]:
        ...
        splits = re.split(r'[,;:]\s+|(?:\s+and\s+)|(?:\s+or\s+)|(?:\s+but\s+)', clean, flags=re.IGNORECASE)
        
        for split in splits:
            split = split.strip()
            if len(split.split()) >= 2:  # Only multi-word phrases
    ...
    def extract_terms(self, sentence: str) -> List[str]:
        """Extract individual terms and technical concepts."""
        terms = []
        ...
        # Extract meaningful word combinations
        words = re.findall(r'\b\w+\b', clean)
        
        # Add 2-word combinations selectively
    ...
    def process_text(self, text: str) -> dict:
        """Process the entire text and return numbered sections."""
    ...
    def format_output(self, breakdown_dict: dict) -> str:
        """Format output to match the expected style."""
        output_lines = []
...
def main():
    """Demonstrate the text breakdown tool with the provided examples."""
    
    # Task 1: .Well-Known URIs
...
Crawlers accelerate discovery but bring scale and risk. Use staged crawling (static -> rendered), prioritize seeds, enforce politeness, and pipeline results into structured exports for analysis."""
    ...
    for section_num, breakdown_list in result1.items():
        formatted_items = [f"'{item}'" for item in breakdown_list[:20]]  # Limit for readability
        print(f"{section_num}. {', '.join(formatted_items)}")
    ...
    for section_num, breakdown_list in result2.items():
        formatted_items = [f"'{item}'" for item in breakdown_list[:20]]  # Limit for readability
        print(f"{section_num}. {', '.join(formatted_items)}")
