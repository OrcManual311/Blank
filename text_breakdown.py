...
into their constituent parts, from large blocks down to individual words
and concepts, following the hierarchical pattern shown in the example.
"""
...
class TextBreakdown:
    ...
    def identify_sections(self, text: str) -> List[str]:
        """
        Identify top-level sections based on double line breaks or section headers.
        """
    ...
    def extract_sentences_and_bullets(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if re.match(r'^[-*•]\s+', line) or re.match(r'^\d+\.\s+', line):
                ...
                # Handle "Header: content" patterns
                sentences.append(line)
            else:
    ...
    def extract_phrases_and_clauses(self, text: str) -> List[str]:
        """
        Break down sentences into main grammatical and conceptual phrases.
        """
        phrases = []
        ...
        # Split by common conjunctions, punctuation, and coordinating words
        # More aggressive splitting to match the example pattern
        splits = re.split(r'[,;:]\s+|(?:\s+and\s+)|(?:\s+or\s+)|(?:\s+but\s+)|(?:\s+that\s+)|(?:\s+which\s+)|(?:\s+such\s+as\s+)|(?:\s+without\s+)|(?:\s+because\s+)', clean_text, flags=re.IGNORECASE)
        
        for split in splits:
            split = split.strip()
            if split and len(split.split()) >= 2:  # Multi-word phrases
                phrases.append(split)
                
                # Further break down longer phrases into smaller components
                if len(split.split()) > 4:
                    # Try to find natural break points
                    sub_splits = re.split(r'\s+(?:from|with|for|to|by|on|at|in)\s+', split, flags=re.IGNORECASE)
                    for sub_split in sub_splits:
                        sub_split = sub_split.strip()
                        if sub_split and sub_split != split and len(sub_split.split()) >= 2:
                            phrases.append(sub_split)
        
        # Also extract parenthetical content
        parenthetical = re.findall(r'\([^)]+\)', text)
        for p in parenthetical:
            p_clean = p.strip('()')
            if p_clean:
                phrases.append(p_clean)
                # Also add the original with parentheses if it's different
                if p != p_clean:
                    phrases.append(p)
        
        return phrases
    
    def extract_key_concepts_and_words(self, text: str) -> List[str]:
        """
        Extract individual words and key concepts, prioritizing technical terms
        ...
        # First, identify multi-word technical concepts and preserve them
        technical_patterns = [
            r'\b[A-Z]{2,}(?:\s+[A-Z]{2,})*\b',          # Acronyms like HTTP, API, OIDC
            r'\b\w+[-_]\w+(?:[-_]\w+)*\b',               # Hyphenated/underscore terms
            r'\b\.well-known\b',                         # Specific technical terms
            r'\bhttps?://[^\s,]+\b',                     # URLs
        ...
        for pattern in technical_patterns:
            ...
            for match in matches:
                ...
                if match:
                    ...
                    found_technical.add(match.lower())
        
        # Extract multi-word phrases (2-3 words) that aren't already captured
        words = re.findall(r'\b\w+\b', clean_text)
        
        # Create 2-word and 3-word combinations for important concepts
        for i in range(len(words)):
            # 2-word phrases
            if i < len(words) - 1:
                two_word = f"{words[i]} {words[i+1]}"
                if (len(two_word) > 6 and  # Skip very short combinations
                    two_word.lower() not in found_technical and
                    not all(w.lower() in self.stop_words for w in two_word.split())):
                    concepts.append(two_word)
                    found_technical.add(two_word.lower())
            
            # 3-word phrases (selective)
            if i < len(words) - 2:
                three_word = f"{words[i]} {words[i+1]} {words[i+2]}"
                # Only include if it contains a non-stop word
                if (any(w.lower() not in self.stop_words for w in three_word.split()) and
                    three_word.lower() not in found_technical):
                    concepts.append(three_word)
                    found_technical.add(three_word.lower())
        
        # Finally, extract individual significant words
        for word in words:
            word_lower = word.lower()
            if (word_lower not in self.stop_words and 
                len(word) > 1 and 
                word_lower not in found_technical):
                concepts.append(word_lower)
                found_technical.add(word_lower)
        
        return concepts
    
    def breakdown_section(self, section_text: str) -> List[str]:
        """
        Perform complete breakdown of a section following the hierarchical approach.
        """
        breakdown = []
        seen = set()  # Track what we've already added
        
        # 1. Capture the entire block first
        if section_text.strip() not in seen:
            breakdown.append(section_text.strip())
            seen.add(section_text.strip().lower())
        
        # 2. Extract and break down sentences/bullet points
        ...
        for sentence in sentences:
            ...
            breakdown.append(sentence)
            seen.add(sentence.lower())
            
            # 3. Break down into phrases and clauses
            phrases = self.extract_phrases_and_clauses(sentence)
            for phrase in phrases:
                phrase = phrase.strip()
                if phrase and phrase.lower() not in seen and phrase != sentence:
                    breakdown.append(phrase)
                    seen.add(phrase.lower())
            
            # 4. Extract key concepts and words
            concepts = self.extract_key_concepts_and_words(sentence)
            for concept in concepts:
                concept = concept.strip()
                if concept and concept.lower() not in seen:
                    breakdown.append(concept)
